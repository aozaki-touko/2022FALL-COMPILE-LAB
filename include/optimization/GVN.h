...
class Expression {
  public:
    ...
    enum gvn_expr_t { e_constant, e_bin, e_phi, e_vari, e_udf};//e_vari means x=a
...
class VarExpression : public Expression{
  public:
    static std::shared_ptr<VarExpression>create(std::shared_ptr<Expression> operand)
    {
      return std::make_shared<VarExpression>(operand);
    }
    virtual std::string print() { return "(var/const: "+rhs->print()+")"; }
    VarExpression(std::shared_ptr<Expression> operand)
        : Expression(e_vari),rhs(operand) {}
  private:
    std::shared_ptr<Expression>rhs;
};
class UdfExpression : public Expression{
  //这一类是调用函数时未确定的值
  public:
  static std::shared_ptr<UdfExpression>create()
  {
    return std::make_shared<UdfExpression>();
  }
    virtual std::string print() { return "Undefined";}  
  UdfExpression():Expression(e_udf){val =nullptr;}
  private:
